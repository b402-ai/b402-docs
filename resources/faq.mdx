...
<Tabs>
    <Tab title="Risks">
        <Accordion title='How does Area manage liquidity risks for Liquid AVS Tokens during market volatility?'>
        ...
        <Accordion title='How resilient is Area and the LAT to disruptions in the EigenLayer ecosystem, such as a major AVS or operator failure?'>
        ...
        <Accordion title='How does Area ensure security for multi-AVS restaking with Liquid AVS Tokens?'>
    ...
    <Tab title="Product">
        <Accordion title='How does Area differentiate from competitors such as Mellow and Stride and maintain its competitive edge?'>
            Area differentiates itself from competitors like Mellow and Stride in both technical and business aspects. Our Liquid AVS Token (LAT) addresses a specific need within the EigenLayer ecosystem that currently lacks native support for individual AVS restaking. Users in EigenLayer must go through operator delegation, which inherently involves AVS diversification. LAT's core functionality is built directly on top of EigenLayer's native smart contracts, giving us a significant advantage in this specific ecosystem.
