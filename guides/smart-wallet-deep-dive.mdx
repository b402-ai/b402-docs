...
| K1 Validator | Configured via `K1_VALIDATOR_ADDRESS` env | ECDSA signature validation module |
...
6. The gas cost (in BNB) is effectively deducted from the payment token amount
...
// Step 1: Request an unsigned UserOp
const verifyRes = await fetch(`${FACILITATOR_URL}/wallet/verify`, {
  method: "POST",
...
console.log(`Fee: ${ethers.formatUnits(feeAmount, 18)} USDT`)
console.log(`Total: ${ethers.formatUnits(totalAmount, 18)} USDT`)

// Step 2: Sign the UserOp hash with the owner key
const ownerWallet = new ethers.Wallet("0xOwnerPrivateKey...")
const signature = await ownerWallet.signMessage(ethers.getBytes(userOpHash))

// Step 3: Submit the signed UserOp
const settleRes = await fetch(`${FACILITATOR_URL}/wallet/settle`, {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({
    userOp,
...
const { success, txHash, payments } = await settleRes.json()
...
Smart wallets can also execute privacy (incognito) operations by wrapping RAILGUN calls in a UserOp. The `/wallet/incognito/verify` endpoint accepts raw contract calls:
...
- [Privacy Deep Dive](/guides/privacy-incognito-deep-dive) - RAILGUN integration details
- [Self-Hosting](/guides/self-hosting) - Run your own facilitator
