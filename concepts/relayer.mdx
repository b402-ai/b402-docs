---
title: b402 Relayer
description: 'On-chain contract for gasless EVM-token payments in b402'
icon: ballot-check
---

The **b402 Relayer** is the on-chain smart contract that enables gasless EVM-token payments on supported networks such as BNB Chain and Base.  
...
Beyond gasless execution, the Relayer supports **multiple tokens within each supported network’s ecosystem**, allowing agents and applications to send and receive payments in custom BEP-20 / ERC-20 assets.
...
- Compatible with all **standard EVM tokens** on supported chains (e.g., BNB Chain and Base)  
