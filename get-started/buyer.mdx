---
title: Buyer Guide
description: 'Send payments to sellers'
icon: 'cart-shopping'
---

The b402 protocol enables users to make gasless payments in EVM-compatible tokens such as USD1 and USDT on supported chains like BNB Chain and Base.
...
1. Node.js 18+
2. A wallet on a supported chain (e.g., BNB Chain or Base) with USD1 tokens
...
This walkthrough uses **BNB Chain mainnet** as the concrete example. To use another supported chain (such as Base), swap in the appropriate RPC endpoint, token address, relayer address, and block explorer URLs.
