...
const EIP712_DOMAIN = {
  name: 'B402',
  version: '1',
  chainId: 56,                    // BNB Chain mainnet (97 for testnet, 8453 for Base)
...
| `chainId` | `56` | BNB Chain mainnet. Use `97` for testnet, `8453` for Base. |
...
| `value` | `uint256` | The transfer amount in the token's smallest unit (wei). For 18-decimal tokens on BNB Chain, `1.0` token = `1000000000000000000`. On Base, USDC uses 6 decimals (`1.0` = `1000000`). |
...
// Convert amount to wei (18 decimals on BNB Chain, 6 for USDC on Base)
...
```bash
curl -X POST https://facilitatorv3.b402.ai/api/v1/verify \
...
```bash
curl -X POST https://facilitatorv3.b402.ai/api/v1/settle \
  -H "Content-Type: application/json" \
...
async function main() {
  ...
  // Step 1: Verify
  console.log('\nVerifying payment...');
  const verifyRes = await fetch(`${FACILITATOR_URL}/api/v1/verify`, {
    method: 'POST',
  ...
  // Step 2: Settle
  console.log('\nSettling payment...');
  const settleRes = await fetch(`${FACILITATOR_URL}/api/v1/settle`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload),
  });
...
- `chainId` must match the network (`56` for mainnet, `97` for testnet)
