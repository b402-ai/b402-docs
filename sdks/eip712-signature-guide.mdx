...
const EIP712_DOMAIN = {
  name: 'B402',
  version: '1',
  chainId: 56,                    // BNB Chain mainnet (97 for testnet)
  verifyingContract: '0x590162bf4b50F6576a459B75309eE21D92178A10', // b402 Relayer
};
```
...
| `chainId` | `56` | BNB Chain mainnet. Use `97` for testnet. |
| `verifyingContract` | `0x590162bf...` | The b402 Relayer contract address. This is the contract that will execute the transfer. |
...
| `value` | `uint256` | The transfer amount in the token's smallest unit (wei). For 18-decimal tokens on BSC, `1.0` token = `1000000000000000000`. |
...
const RELAYER_ADDRESS = '0x590162bf4b50F6576a459B75309eE21D92178A10';
...
// Convert amount to wei (18 decimals on BSC)
const amount = ethers.parseUnits('0.01', 18); // 0.01 tokens
...
```bash
curl -X POST https://facilitator.b402.ai/api/v1/verify \
...
```bash
curl -X POST https://facilitator.b402.ai/api/v1/settle \
  -H "Content-Type: application/json" \
...
**Fix:** All supported tokens (USDT, USDC, USD1) use 18 decimals on BNB Chain. Always use `ethers.parseUnits()`:

```typescript
// Correct: 0.01 tokens with 18 decimals
const value = ethers.parseUnits('0.01', 18).toString();
// '10000000000000000'

// Wrong: using 6 decimals (this is NOT BSC)
const wrong = ethers.parseUnits('0.01', 6).toString();
// '10000' - too small, payment will fail or be incorrect
```
...
- [b402-sdk](/sdks/b402-sdk) - Use the SDK to handle signatures automatically
