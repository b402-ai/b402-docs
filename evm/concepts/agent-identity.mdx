...
Both the [EOA](/evm/concepts/wallet#eoa-wallets) and [Smart Wallet](/evm/concepts/wallet#smart-wallets-erc-7579-nexus) payment flows work with or without an ERC-8004 identity. The identity adds a trust layer on top of the existing payment infrastructure.

<Note>
ERC-8004 is EVM-specific. On Solana, agent identity is expressed implicitly: each agent's spending authority is a Solana keypair, and inside the shielded pool an agent appears as a PDA derived from a private spending key (not as the underlying wallet). A cross-chain identity layer is on the roadmap.
</Note>

## Learn More

- [Wallet Concepts](/evm/concepts/wallet) - EOA and smart wallet payment models
- [ERC-8004 Specification](https://eips.ethereum.org/EIPS/eip-8004) - Full Ethereum standard
- [Quickstart](/evm/quickstart) - Get started with b402 SDK and ClawPay
