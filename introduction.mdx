...
description: 'Unified agentic payments rails'
icon: star
---

b402 is a complete internet-native payment infrastructure built for modern EVM chains, starting with the [BNB Chain](https://www.bnbchain.org/en) and [Base](https://base.org), inspired by the `HTTP 402` standard and [x402](https://x402.org) protocol.

It enables HTTP-native payment requests that support multi-token transactions, bringing frictionless payment capabilities to the BNB and Base ecosystems.
...
* **EVM-native deployment** with first-class support for BNB Chain and Base
...
b402 unlocks a new category of agent-native applications on EVM chains like BNB Chain and Base:
