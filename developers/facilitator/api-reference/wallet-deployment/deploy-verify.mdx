...
```bash
curl -X POST https://facilitator.b402.ai/deploy/verify \
...
```typescript
const response = await fetch("https://facilitator.b402.ai/deploy/verify", {
  method: "POST",
...
```bash
curl -X POST https://facilitator.b402.ai/deploy/wallet/verify \
  -H "Content-Type: application/json" \
  -d '{
    "paymentRequirements": {
      "network": "bsc",
...
```typescript
const response = await fetch("https://facilitator.b402.ai/deploy/wallet/verify", {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({
    paymentRequirements: {
