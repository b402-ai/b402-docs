...
```bash
curl -X POST https://facilitator.b402.ai/deploy/settle \
    ...
    "paymentPayload": [
      {
        ...
        "payload": {
          "authorization": {
            ...
            "to": "0x590162bf4b50F6576a459B75309eE21D92178A10",
...
const settleResponse = await fetch("https://facilitator.b402.ai/deploy/settle", {
  method: "POST",
...
```bash
curl -X POST https://facilitator.b402.ai/deploy/wallet/settle \
  -H "Content-Type: application/json" \
  -d '{
...
const settleResponse = await fetch("https://facilitator.b402.ai/deploy/wallet/settle", {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({
