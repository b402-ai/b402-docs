...
`POST /wallet/incognito/settle` takes the signed UserOperation from [`/wallet/incognito/verify`](/developers/facilitator/api-reference/incognito/verify) and submits it to the ERC-4337 bundler for on-chain execution. The flow is identical to standard wallet settlement but executes a RAILGUN shield or unshield operation.
...
```bash
curl -X POST https://facilitator.b402.ai/wallet/incognito/settle \
  -H "Content-Type: application/json" \
  -d '{
    "userOp": {
      "sender": "0x...wallet_address",
      "nonce": "0x...",
      "callData": "0x...encoded_railgun_call",
      "callGasLimit": "0x...",
...
const verifyRes = await fetch('https://facilitator.b402.ai/wallet/incognito/verify', {
  method: 'POST',
...
const settleRes = await fetch('https://facilitator.b402.ai/wallet/incognito/settle', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({
