...
| `signature_error` | 401 | EIP-712 signature invalid | Wrong signer, corrupted signature, wrong chain ID |
...
| `userp_error` | 500 | UserOperation failed | Bundler rejection, gas estimation failure, paymaster error |
...
| Error | Message |
|-------|---------|
| `ERR_SWAP_QUOTE_FAILED` | Failed to get swap quote |
| `ERR_SWAP_EXECUTION_FAILED` | Swap execution failed |
| `ERR_SWAP_INSUFFICIENT_LIQUIDITY` | Insufficient liquidity for swap |
| `ERR_SWAP_SLIPPAGE_EXCEEDED` | Swap slippage exceeded maximum allowed |
| `ERR_SWAP_MISSING_PARAMS` | Missing required swap parameters |
| `ERR_SWAP_INVALID_TOKEN` | Invalid token address for swap |
